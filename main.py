...
def main_interface():
    ...
    btn_booking = tk.Button(root, text="🅿️ Book a Slot", font=("Arial", 16), command=booking_interface, width=20,
                            height=2, bg="#007BFF", fg="white", relief="flat")
    btn_booking.pack(pady=10)
    btn_booking.bind("<Enter>", on_enter)
    btn_booking.bind("<Leave>", on_leave)

    btn_simulation = tk.Button(root, text="🎬 Run Simulation", font=("Arial", 16), command=simulation_interface,
                               width=20, height=2, bg="#28A745", fg="white", relief="flat")
    btn_simulation.pack(pady=10)
...
def booking_interface():
    ...
    def generate_ticket():
        ...
        if not (name and slot and hour and minute and am_pm and duration.isdigit()):
    ...
    tk.Button(booking_root, text="📥 Book Now", font=("Arial", 14, "bold"), command=generate_ticket, bg="#007BFF",
              fg="white", relief="flat").pack(pady=20)
...
def simulation_interface():
    root.destroy()
    sim_root = tk.Tk()
    sim_root.title("🅿️ Parking Simulation")
    sim_root.geometry("800x500")
    sim_root.configure(bg="#1E1E1E")

    tk.Label(sim_root, text="🎬 Choose Test Case", font=("Arial", 18, "bold"), bg="#1E1E1E", fg="white").pack(pady=20)

    for i in range(1, 4):
        tk.Button(sim_root, text=f"Test Case {i}", font=("Arial", 14), bg="#FF5733", fg="white").pack(pady=10)
